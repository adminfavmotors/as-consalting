...
export const privacySections = [
  ...
  ['cele', 'Cele i podstawy prawne', `
    <ul>
      ...
      <li><strong>Statystyki Google Analytics po aktywacji usługi:</strong> art. 6 ust. 1 lit. a RODO — dobrowolna zgoda na kategorię statystyczną.</li>
      <li><strong>Zapamiętanie i udokumentowanie wyboru cookies:</strong> art. 6 ust. 1 lit. c RODO — obowiązki związane z wykazaniem zgody i respektowaniem Twojego wyboru.</li>
  ...
  ['odbiorcy', 'Odbiorcy danych i hosting', `
    <p>Do danych mogą mieć dostęp dostawcy poczty elektronicznej, hostingu i obsługi technicznej, a po aktywacji odpowiednich funkcji również Cookiebot by Usercentrics oraz Google, w zakresie potrzebnym do świadczenia swoich usług, oraz uprawnione organy, jeżeli wymagają tego przepisy.</p>
  ...
  ['transfery', 'Przetwarzanie poza EOG', `
    ...
    <p>Po włączeniu statystyk dane są przetwarzane także w infrastrukturze Google, co może obejmować transfer poza EOG. Informacje Google o przetwarzaniu i transferach znajdują się w <a href="https://policies.google.com/privacy?hl=pl">polityce prywatności Google</a>. Obsługa zgód podlega warunkom Cookiebot by Usercentrics. Zakres transferów należy oceniać według usług i umów wykorzystywanych przez administratora.</p>
  ...
  ['cookies', 'Cookies, analityka i profilowanie', `
    <p>${getConsentConfig().enabled ? 'Strona jest skonfigurowana do korzystania z Cookiebot i Google Analytics 4 na wskazanej domenie.' : 'W tej wersji strony Google Analytics i Cookiebot nie są jeszcze aktywne.'} Po aktywacji usługi Google Analytics ładuje się dopiero po wyrażeniu zgody na statystyki w panelu Cookiebot. Odmowa nie ogranicza działania strony.</p>
    <p>Zgodę można zmienić lub wycofać przez <a href="/cookies/#ustawienia" data-cookie-settings>ustawienia cookies</a>. Wycofanie nie wpływa na zgodność z prawem przetwarzania sprzed wycofania. Opis kategorii, dostawców i cookies znajduje się na stronie <a href="/cookies/">Polityka cookies</a>.</p>
    <p>Nie dodajemy Google Ads ani nagrywania sesji. Pliki czcionek i zdjęć pozostają lokalne. Kod analityczny nie przekazuje treści formularza ani parametrów zapytania i fragmentów adresu strony.</p>
    ...
    body: `<nav class="breadcrumbs wrap" aria-label="Ścieżka nawigacji"><ol><li><a href="/">Strona główna</a></li><li aria-current="page">Polityka prywatności</li></ol></nav>
      ...
      <div class="wrap privacy-layout">
        ...
        <article class="privacy-copy" aria-label="Treść polityki prywatności">
          <aside class="privacy-draft" aria-label="Status dokumentu"><strong>Projekt do weryfikacji</strong><p>Przed przyjęciem dokumentu administrator powinien potwierdzić zasady przechowywania korespondencji, dostawcę poczty, ustawienia retencji GA4 oraz warunki usług i transferów danych. Ten projekt nie jest zatwierdzoną polityką.</p></aside>
